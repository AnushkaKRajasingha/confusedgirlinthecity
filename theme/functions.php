 ...
 */load_template( trailingslashit( get_template_directory() ) . 'inc/theme-options.php' );
	...
	add_action( 'wp_ajax_submitPopupSubscription', 'submitPopupSubscription' );
	add_action( 'wp_ajax_nopriv_submitPopupSubscription', 'submitPopupSubscription' );
	
	function submitPopupSubscription() {
		wp_mail( get_option( 'admin_email' ), 'You have a new subscriber - '.get_bloginfo('name'), 'Subscriber email address is '.$_POST['email'].PHP_EOL.get_bloginfo('name') );	
