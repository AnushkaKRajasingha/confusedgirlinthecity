(function ($) {
    ...
    function submitEmailSubscription(){
    	jQuery.post(
    			window._popup_data.ajaxurl +"?action=submitPopupSubscription",
				{	
					email : $('#subs-email').val()
				},
				function( response ) {
					$('#subs-email').val('');
					$('#subs-msg').html('Thanks for Subscribing ..!');
					setTimeout(function(){
						$('#subs-msg').html('');
					}, 3000);
				}
    ...
    $(document).ready(function () {
       ...
       setTimeout(function(){
    	   	$('#subs_submit').click(submitEmailSubscription);
        }, 6000);
